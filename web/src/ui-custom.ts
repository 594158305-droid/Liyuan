/**
 * 自定义 UI（页面宽度 + 字体比例）：localStorage 持久化，行内 CSS 变量即时生效。
 * 字体比例用 html zoom 整体缩放（全站 px 硬编码，逐处变量化不现实）；
 * zoom 与宽度乘算，宽度反算 --chat-w = 设定px / zoom 保持视觉宽度准确。
 */
...
export const UI_DEFAULTS: UiCustom = { chatW: 900, fontScale: 100 };
	...
	try {
		...
		if (raw && typeof raw === "object") {
			...
			if (Number.isFinite(chatW) && Number.isFinite(fontScale)) {
				return { chatW: clamp(chatW, UI_CHAT_W_MIN, UI_CHAT_W_MAX), fontScale: clamp(fontScale, UI_FONT_MIN, UI_FONT_MAX) };
			}
		}
	} catch {
