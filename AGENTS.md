...
## 铁律（upstream 合并引入，与下方「排查纪律」共同约束）

1. **禁止直接动手改代码**，除非用户明确说「开始/动手」。用户会说「先同步情况」「你来告诉我」「只读」——那期间只能读、不能改。
2. **读思考记录必须先读** `docs/READING-THINKING.md`——按文件 mtime 找最新会话会读错位置（旧会话被 model_change 碰过 mtime 会排到前面）。必须用行级 timestamp 核对北京时间。
3. **流程与提示词的最终形态**定义在 `docs/PLAN-ROUND-FLOW.md`——任何提示词/引擎改动都要回答「离这个流程近了多少」。
4. 预设拆层规则见 `docs/PRESET-SPLIT-TAXONOMY.md`；RP agent 执行计划见 `docs/PLAN-RP-AGENT-EXEC.md`。
...
- **诊断当前聊天优先查主聊天跟踪**（2026-08-11）：需要诊断当前聊天的情况时，先看 `.liyuan-state/trace/<sessionId>.jsonl` 有没有对应的信息可以用来定位——主聊天跟踪（设置 → 开发者模式 → 主聊天跟踪）开启后，每拍记录送模提示词/思考/工具调用/草稿/旁路模型/定稿全文及角色卡/预设/时间元信息（事件 schema 见 `docs/DESIGN-debug-trace.md`）。文件按会话分，未开启时无此文件。
