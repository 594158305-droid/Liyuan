...
❌ 仍不兼容：完整 STscript、脚本直改正文、依赖酒馆 DOM/主题的前端插件；经 UI 的显式改稿受支持——带「已改写」标记、原文可回滚。
...
- 剧情正文：自动路径永远是模型的原始输出——梨园的代码与辅助模型只做输入侧加工、结构化记账和元信息标注，绝不改写、补写正文；显式改稿（用户手改 / 经你同意由助手改）走 rp-edited 分支条目，带标记、原文可回滚。修音可以，假唱不行。
