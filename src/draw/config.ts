/**
 * 生图配置（liyuan.draw.json）：provider 注册表 + 参数预设 + 全局风格 + 默认值。
 *
...
/** 全局风格预设（与 provider / 角色无关的全局层，作用于画面整体观感） */
export interface DrawStyle {
	id: string;
	name: string;
	/** 正面前缀 tag 串（可带 n::tag:: 权重，与角色无关的全局层） */
...
export interface SdWebUiConfig {
	host: string;
	auth: string;
	/** 传输方式：st-proxy（脚本代理）/ direct（直连 sd-webui） */
	transport: "st-proxy" | "direct";
	model: string;
}

/** ComfyUI 专属配置（type="comfyui" 时生效） */
export interface ComfyUiConfig {
	/** 连接方式：proxy（经网关）/ direct（直连） */
	connectionMode: "proxy" | "direct";
	url: string;
	auth: string;
	workflow: string;
	/** 工作流中的节点 id 映射（如 {"positive": "6", "negative": "7"}） */
...
export interface DrawProvider {
	...
	autoConfirm: boolean;
	/** SD WebUI 专属配置（type="sd-webui" 时有效） */
	sd?: SdWebUiConfig;
	/** ComfyUI 专属配置（type="comfyui" 时有效） */
	comfy?: ComfyUiConfig;
}
...
export function normalizeDrawProvider(raw: unknown): DrawProvider | null {
	...
	// type 专属配置：sd-webui / comfyui 分别按各自结构逐字段校验，其余 type 忽略
	let sd: SdWebUiConfig | undefined;
	let comfy: ComfyUiConfig | undefined;
	if (type === "sd-webui") {
		const s = (o.sd && typeof o.sd === "object" ? o.sd : {}) as Record<string, unknown>;
		sd = {
			host: typeof s.host === "string" && s.host.trim() ? s.host.trim() : "http://127.0.0.1:7860",
			auth: typeof s.auth === "string" ? s.auth : "",
			transport: s.transport === "direct" ? "direct" : "st-proxy",
			model: typeof s.model === "string" ? s.model : "",
		};
	} else if (type === "comfyui") {
		const c = (o.comfy && typeof o.comfy === "object" ? o.comfy : {}) as Record<string, unknown>;
		const nodeIds = c.nodeIds && typeof c.nodeIds === "object" && !Array.isArray(c.nodeIds)
			? Object.fromEntries(
					Object.entries(c.nodeIds as Record<string, unknown>).filter(([, v]) => typeof v === "string") as [string, string][],
				)
			: {};
		comfy = {
			connectionMode: c.connectionMode === "direct" ? "direct" : "proxy",
			url: typeof c.url === "string" && c.url.trim() ? c.url.trim() : "http://127.0.0.1:8188",
			auth: typeof c.auth === "string" ? c.auth : "",
...
/** 规范化单条全局风格（id 必填非空；name 缺省=id；prefix 缺省=""；非法返回 null） */
export function normalizeDrawStyle(raw: unknown): DrawStyle | null {
	if (!raw || typeof raw !== "object") return null;
	const o = raw as Record<string, unknown>;
	const id = typeof o.id === "string" && o.id.trim() ? o.id.trim() : "";
	if (!id) return null;
	...
	return {
		id,
		name,
		positivePrefix: typeof o.positivePrefix === "string" ? o.positivePrefix : "",
		negativePrefix: typeof o.negativePrefix === "string" ? o.negativePrefix : "",
	};
}
...
export function loadDrawConfig(cwd: string): DrawConfig {
	...
	try {
		...
		const styles = Array.isArray(raw.styles)
			? raw.styles.map(normalizeDrawStyle).filter((s): s is DrawStyle => s !== null)
			: [];
		const defaultStyleId =
			typeof raw.defaultStyleId === "string" &&
			raw.defaultStyleId &&
			styles.some((s) => s.id === raw.defaultStyleId)
				? raw.defaultStyleId
				: styles[0]?.id ?? "";
		return {
			version: 1,
