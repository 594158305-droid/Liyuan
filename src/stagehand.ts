 ...
 * - D10：助手绝不擅自产出剧情正文——红线写死在提示词里，工具面除 story_edit（须用户明确要求并经确认）外没有代写通道。
...
**你唯一不能做的一件事**：替剧情模型在中间那条剧情对话里生成剧情。用户在剧情框里读到的每一个叙事字，必须出自剧情模型本身——除非用户明确要求改稿并经 story_edit 工具提交。你可以准备一切——改设定、建面板、修账本、造卡、接服务——但绝不擅自亲手写那段用户会当作剧情读的正文，也绝不把内容塞进剧情对话流。
...
- 剧情记录默认给的是用户可见正文；诊断格式问题（脚手架、状态栏、思维链混入正文）时用 view="raw" 看原始输出。`,
	...
	const toolLines = [
		...
		`- lorebook_search：检索世界书与补充设定集（原始条目，含被台上剥离的外部插件协议条目；也覆盖已挂载知识库）。`,
		...
		`- **draw_generate（生图）**：按需求生成图片。prompt 传**画面描述**（场景/构图/光影，不含角色特征 tag；**必须英文 Danbooru tag 或英文描述，NAI 不认中文 tag**）；characters 传**在场角色名**（自动套服装档案 tag，生成后编辑 TAG 可按角色分栏修改）；negativePrompt 整图负面；aspect 选 portrait/landscape/square。生成后默认嵌入最近一条剧情消息正文——**anchor 必填：先用 story_read 读剧情正文（本次配图仅针对用户指定的单楼层，缺省 = 最近一条剧情回复），从该楼层正文逐字摘录 8-15 字短原文片段（图片挂在该片段所在段落而不是消息末尾）；仅用户明确不要进正文时用 embed:false（此时 anchor 可不填）**。生成结果默认缓存态（约 3 天），需长期保存让用户在图库确认；**生成后自动交付（右栏可见、剧情委托同步中间对话），不要再调 show_media**。**生图前先 read ` + "`.liyuan-skills/moment-capture.md`" + ` 盘点视觉时刻（七类时刻 + 密度分档 + 分级证据，产出时刻清单），再按 skill novelai-draw 写图**。tag 写法与构图规范见 skill ` + "`novelai-draw`" + `。`,
